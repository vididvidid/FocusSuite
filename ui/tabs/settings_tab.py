...
class SettingsTab(ttk.Frame):
    ...
    def _create_api_frame(self):
        api_frame=ttk.LabelFrame(self, text="API Configuration", padding=10)
        ...
        test_api_button = ttk.Button(api_frame, text="Save a Test", command=self.callbacks['test_api'])
        test_api_button.grid(row=0,column=2, padx=5)

        ttk.Label(api_frame, text="Worker Endpoint URL: ").grid(row=1, column=0, sticky='w', pady=2)
        self.worker_url_entry = ttk.Entry(api_frame, width=50)
        self.worker_url_entry.grid(row=1, column=1, padx=5, pady=2, sticky='ew')
    ...
    def _create_appearance_frame(self):
        ...
        dark_rb = ttk.Radiobutton(theme_frame, text="Dark Mode",
                                  variable=self.theme_var, value="dark", command=lambda:self.theme_applier("dark"))
